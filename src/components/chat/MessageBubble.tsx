...
function CopyMsgButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handle}
      className="flex h-8 w-8 items-center justify-center rounded-lg text-muted opacity-0 transition-opacity group-hover:opacity-100 hover:bg-hover hover:text-primary"
      aria-label="复制消息"
    >
...
export function MessageBubble({ message }: { message: Message }) {
  ...
  if (message.role === "user") {
    return (
      <div ref={ref} className="group flex items-end justify-end gap-2">
